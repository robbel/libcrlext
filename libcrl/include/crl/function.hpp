...
class _DiscreteFunction {
  ...
  friend DiscreteFunction<U> algorithm::join_Shared(cpputil::Iterator<DiscreteFunction<U>>& funcs, BinOp binOp);
  template<class F, class U, class BinRefOp>
  friend DiscreteFunction<U> algorithm::genericOp_Shared(const _DiscreteFunction<U>* pf, SizeVec vars, bool known_flat, U init, BinRefOp binOp);
  // operator overloads
