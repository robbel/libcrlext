...
Run as ./gtopdf3 <graph_file> <pdf_file>
"""
import numpy as np
...
if __name__ == "__main__":
  if len(sys.argv) < 4:
    print "Usage: python gtopdf3.py <config> <graph_file> <pdf_file>"
    sys.exit(0)
  ...
  #graph_draw(g, pos=pos, vertex_text=g.vertex_index, vertex_font_size=8, vertex_size=deg, vertex_fill_color=view, vorder=deg, edge_color=ebet, eorder=eorder, edge_pen_width=ebet, edge_control_points=control, output="graph-draw.pdf")
  #graph_draw(g, pos=pos, vertex_text=g.vertex_index, vertex_font_size=8, vertex_size=deg, vertex_fill_color=bv, vorder=deg, edge_color=ebet, eorder=eorder, edge_pen_width=be, edge_control_points=control, ecmap=matplotlib.cm.gist_heat_r, output="graph-draw.pdf")
  graph_draw(g, pos=pos, vertex_text=g.vertex_index, vertex_font_size=8, vertex_size=deg, vorder=deg, vertex_shape=view, vertex_fill_color=bv, edge_color=ebet, eorder=eorder, edge_pen_width=be, edge_control_points=control, ecmap=matplotlib.cm.gist_heat_r, vcmap=matplotlib.cm.gist_heat_r, output="graph-draw.pdf")
